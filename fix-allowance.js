async function main() {
  ...
  const l2 = new ClobClient('https://clob.polymarket.com', 137, wallet,
    { key: creds.key, secret: creds.secret, passphrase: creds.passphrase }, 1, funder);
  
  console.log('Checking allowances...');
  const bal = await l2.getBalanceAllowance({ asset_type: 'COLLATERAL' });
  console.log('Balance:', parseInt(bal.balance) / 1e6);
  console.log('Allowances:', JSON.stringify(bal.allowances));
  
  console.log('Setting allowances...');
  const result = await l2.updateBalanceAllowance({ asset_type: 'COLLATERAL' });
